"""CreditCycleScheduler — weekly ingestion loop for credit-cycle-v1."""
...
from .domain import CreditCycleV1
from .ingestion.fred_client import FREDClient
from .ingestion.pipeline import CreditCyclePipeline, _last_friday, _weekly_backfill_dates

logger = logging.getLogger(__name__)
_DEFAULT_RUN_HOUR_UTC = 9


class CreditCycleScheduler:
    """Weekly ingestion scheduler for credit-cycle-v1."""

    def __init__(
        self,
        engine: ProbabilisticOntologyEngine,
        pipeline: CreditCyclePipeline,
    ...
    async def run_once(self, target_date: date | None = None) -> bool:
        ...
        try:
            ...
            logger.error("Credit cycle ingestion failed for %s: %s", target_date, exc)
            return False
        ...
        if domain_id:
            try:
                ...
                logger.error("Learning failed for credit-cycle-v1 @ %s: %s", target_date, exc)
        return True
    ...
    async def run_forever(self) -> None:
        if self._backfill_weeks > 0:
            ...
            logger.info("Credit cycle backfill complete: %d weeks ingested", n)
        while True:
...
if __name__ == "__main__":
    asyncio.run(_main() if False else None)
