"""EnergyRegimeScheduler — weekly ingestion loop for energy-regime-v1."""
...
from .domain import EnergyRegimeV1
from .ingestion.fred_client import FREDClient
from .ingestion.yfinance_client import EnergyYFinanceClient
from .ingestion.pipeline import EnergyRegimePipeline, _last_friday, _weekly_backfill_dates

logger = logging.getLogger(__name__)
_DEFAULT_RUN_HOUR_UTC = 9


class EnergyRegimeScheduler:
    """Weekly ingestion scheduler for energy-regime-v1."""

    def __init__(
        self,
        engine: ProbabilisticOntologyEngine,
        pipeline: EnergyRegimePipeline,
    ...
    async def run_once(self, target_date: date | None = None) -> bool:
        ...
        try:
            ...
            logger.error("Energy regime ingestion failed for %s: %s", target_date, exc)
            return False
        ...
        if domain_id:
            try:
                ...
                logger.error("Learning failed for energy-regime-v1 @ %s: %s", target_date, exc)
        return True
    ...
    async def run_forever(self) -> None:
        if self._backfill_weeks > 0:
            ...
            logger.info("Energy regime backfill complete: %d weeks ingested", n)
        while True:
