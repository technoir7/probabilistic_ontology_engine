"""
NASDAQClient — fetches ZS (soybean) front-month futures settlement prices via
the Nasdaq Data Link (formerly Quandl) REST API.

API
---
    GET https://data.nasdaq.com/api/v3/datasets/CHRIS/CME_S1.json
        ?api_key=<NASDAQ_API_KEY>
        &rows=<N>

    Dataset  : CHRIS/CME_S1  — CBOT Soybean Futures, Continuous Front Month
...
Derived variable
----------------
    SoyPriceUp = settle_cents_per_bushel > rolling_20d_avg_cents
...
_BASE_URL      = "https://data.nasdaq.com/api/v3/datasets/CHRIS/CME_S1.json"
_PRICE_ROWS    = 21   # 1 latest + 20 for rolling average
_SETTLE_INDEX  = 4    # column index of "Settle" in CHRIS/CME_S1 data rows
...
@dataclass
class SoybeanNASDAQSnapshot:
    target_date: date
    settle_cents_per_bushel: float   # latest ZS front-month settlement price
...
class NASDAQClient:
    """
    Asynchronous client for Nasdaq Data Link (formerly Quandl) CHRIS/CME_S1.
    ...
    async def fetch_snapshot(self, target_date: date) -> SoybeanNASDAQSnapshot:
        """
        Fetch the last _PRICE_ROWS days of ZS settlement prices and return
        a SoybeanNASDAQSnapshot.
    ...
    @staticmethod
    def build_snapshot(target_date: date, rows: list[list]) -> SoybeanNASDAQSnapshot:
        """
        Map raw Nasdaq Data Link CHRIS/CME_S1 data rows to a SoybeanNASDAQSnapshot.
        ...
        if len(rows) < 2:
            raise IOError(
                f"Nasdaq CME_S1 returned {len(rows)} row(s); need ≥2 to compute "
                "the 20-day rolling average."
            )

        prices = _extract_settle_prices(rows)
        if not prices:
            raise IOError("No valid settlement prices in Nasdaq CME_S1 response")
    ...
    async def _fetch_settle_rows(self) -> list[list]:
        """
        Fetch _PRICE_ROWS most-recent rows from CHRIS/CME_S1.
        ...
        try:
            ...
            raise IOError(
                f"Nasdaq API HTTP {exc.response.status_code} for CHRIS/CME_S1"
            ) from exc
        except Exception as exc:
            raise IOError(f"Nasdaq API request failed for CHRIS/CME_S1: {exc}") from exc

        data = body.get("dataset", {}).get("data", [])
        if not data:
            raise IOError("Nasdaq API returned empty data array for CHRIS/CME_S1")
        return data
...
def _extract_settle_prices(rows: list[list]) -> list[float]:
    ...
    for row in rows:
        try:
            ...
            logger.warning("Skipping CME_S1 row with bad settle value: %s", exc)
    return out
