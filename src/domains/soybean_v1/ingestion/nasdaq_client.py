"""
NASDAQClient — fetches ZS (soybean) front-month futures prices via yfinance.

The class name and snapshot shape are retained for compatibility with the
existing soybean ingestion pipeline.

Market data
-----------
    ticker   : ZS=F  — CBOT soybean front-month futures
    source   : Yahoo Finance via yfinance
    interval : daily

Derived variable
----------------
    SoyPriceUp = latest_close_cents_per_bushel > rolling_20d_avg_cents
...
class SoybeanNASDAQSnapshot:
    target_date: date
    settle_cents_per_bushel: float   # latest ZS front-month close price
...
class NASDAQClient:
    """
    Asynchronous wrapper around yfinance.download for ZS=F.
    ...
    async def fetch_snapshot(self, target_date: date) -> SoybeanNASDAQSnapshot:
        """
        Fetch one month of daily ZS=F prices and return a SoybeanNASDAQSnapshot.
    ...
    @staticmethod
    def build_snapshot(target_date: date, history: Any) -> SoybeanNASDAQSnapshot:
        """
        Map yfinance daily price history to a SoybeanNASDAQSnapshot.
    ...
    async def _fetch_price_history(self) -> Any:
        """
        Fetch daily ZS=F price history.
