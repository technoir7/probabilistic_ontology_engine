"""
SoybeanPipeline — combines USDA NASS and yfinance observations
...
----------
    Both sources are authoritative primary data:
        ...
        yfinance ZS=F → confidence = 1.0

Usage
-----
    pipeline = SoybeanPipeline(nass_client, nasdaq_client)
...
class SoybeanPipeline:
    """
    Fetches USDA NASS and yfinance ZS=F data concurrently and
    ...
    async def fetch_evidence(self, target_date: date) -> EvidenceRecord:
        ...
        logger.info(
            ...
            "YieldForecastDown=%s(%s) SoyPriceUp=%s(%s)",
            week_end,
    ...
    def build_evidence_record(
        ...
        """
        Map SoybeanNASSSnapshot + SoybeanNASDAQSnapshot to an EvidenceRecord.
...
def _nass_soft_assignment(
    *,
    ...
    When ``is_observed`` is True but ``p_true`` is None the assignment falls
    back to hard OBSERVED.
    """
    if not is_observed:
        return ObservedAssignment(
            variable_id=variable_id,
            observed_value=False,   # sentinel; missingness=MISSING is authoritative
