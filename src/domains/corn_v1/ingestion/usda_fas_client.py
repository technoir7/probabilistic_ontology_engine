"""
USDAFASClient — fetches weekly U.S. corn export inspection volume from the
...
---
    GET https://apps.fas.usda.gov/gats/ExpressQuery1.aspx
        ?formatType=json
        &commodity=0440000   # USDA commodity code for corn
        &unit=MT             # metric tons
        &tradeType=E         # exports
        &period=W            # weekly
        &year=<YYYY>
...
_CORN_CODE      = "0440000"   # USDA FAS commodity code for corn
...
class USDAFASClient:
    """
    Asynchronous client for USDA FAS weekly corn export data.
    ...
    async def fetch_snapshot(self, target_date: date) -> CornFASSnapshot:
        """
        Fetch the most recent _WEEKS_NEEDED weeks of corn export inspection
        data and return a CornFASSnapshot.
    ...
    @staticmethod
    def build_snapshot(target_date: date, rows: list[dict]) -> CornFASSnapshot:
        """
        Map raw FAS API rows to a CornFASSnapshot.  Static and synchronous
