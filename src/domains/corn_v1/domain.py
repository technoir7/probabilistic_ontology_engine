"""
corn-v1 — U.S. corn (ZC) market domain module.

Variables (all BOOLEAN):
    PlantingDelayed     True  = current corn planting progress is more than 5 percentage
    ...
    YieldForecastDown   True  = the most recent USDA NASS/WASDE corn yield forecast
...
def get_variables() -> dict[str, Variable]:
    """Return the shared canonical variable set for corn-v1."""
...
def make_weather_dominant_candidate(module_id: str = "corn-v1") -> OntologyCandidate:
    """
    W*: PlantingDelayed → YieldForecastDown ← DroughtIndex → YieldForecastDown → CornPriceUp
    ...
    return OntologyCandidate(
        ...
        edges=[
            ...
            _edge("YieldForecastDown","CornPriceUp",        prior=0.65),
        ],
...
def make_null_candidate(module_id: str = "corn-v1") -> OntologyCandidate:
    """
    Null: YieldForecastDown → CornPriceUp only.
    ...
    return OntologyCandidate(
        ...
        variables=_var_list(),
        edges=[
            _edge("YieldForecastDown", "CornPriceUp", prior=0.55),
        ],
